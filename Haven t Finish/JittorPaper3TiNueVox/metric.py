...
class MSE(object):
    def __call__(self, pred, gt):
        return jt.mean((pred - gt) ** 2)

# Peak Signal to Noise Ratio
class PSNR(object):
    def __call__(self, pred, gt):
        mse = jt.mean((pred - gt) ** 2)
        return 10 * -jt.log(mse) / jt.log(10)
...
class SSIM(object):
    ...
    def gaussian(self, w_size, sigma):
        gauss = jt.array([math.exp(-(x - w_size//2)**2/float(2*sigma**2)) for x in range(w_size)])
    ...
    def __call__(self, y_pred, y_true, w_size=11, size_average=True, full=False):
        ...
        if jt.max(y_pred) > 128:
            max_val = 255
        else:
            max_val = 1

        if jt.min(y_pred) < -0.5:
            min_val = -1
        else:
            min_val = 0
        L = max_val - min_val
        ...
        window = self.create_window(w_size, channel=channel)

        mu1 = nn.conv2d(y_pred, window, padding=padd, groups=channel)
        mu2 = nn.conv2d(y_true, window, padding=padd, groups=channel)

        mu1_sq = mu1.pow(2)
        mu2_sq = mu2.pow(2)
        mu1_mu2 = mu1 * mu2

        sigma1_sq = nn.conv2d(y_pred * y_pred, window, padding=padd, groups=channel) - mu1_sq
        sigma2_sq = nn.conv2d(y_true * y_true, window, padding=padd, groups=channel) - mu2_sq
        sigma12 = nn.conv2d(y_pred * y_true, window, padding=padd, groups=channel) - mu1_mu2
