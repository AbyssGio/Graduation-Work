...
class Load_hyper_data():
    def __init__(self, 
        ...
        if use_bg_points:
            ...
            self.bg_points = jt.float32(self.bg_points)
    ...
    def load_raw(self, idx):
        ...
        rays_dir = jt.array(camera.pixels_to_rays(pixels)).float().view([-1,3])
        rays_ori =jt.array(camera.position[None, :]).float().expand_as(rays_dir)
        rays_color = jt.array(np.array(image)).view([-1,3])/255.
        return {'rays_ori': rays_ori, 
                'rays_dir': rays_dir, 
                'viewdirs':rays_dir / rays_dir.norm(dim=-1, keepdim=True),
                'rays_color': rays_color, 
                'near': jt.array(self.near).float().view([-1]),
                'far': jt.array(self.far).float().view([-1]),}
