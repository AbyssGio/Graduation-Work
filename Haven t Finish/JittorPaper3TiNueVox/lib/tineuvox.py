...
class Deformation(nn.Module):
    ...
    def create_net(self):
        ...
        layers[0] = wn.weight_norm(layers[0],name="weight", dim=1)
        ...
        for i in range(self.D - 2):
            ...
            sine_init(layers[-1])
            layers[-1] =  wn.weight_norm(layers[-1],name="weight", dim=1)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, heads, d_model, dropout = 0.1):
        ...
        self.q_linear = jt.nn.Linear(d_model, d_model)
        self.v_linear = jt.nn.Linear(d_model, d_model)
        self.k_linear = jt.nn.Linear(d_model, d_model)
        self.dropout = jt.nn.Dropout(dropout)
        self.out = jt.nn.Linear(d_model, d_model)
...
class TiNeuVox(nn.Module):
    def __init__(self, xyz_min, xyz_max,
        ...
        timenet_depth = 1
        timenet_outputs = [1,10,20,voxel_dim+voxel_dim*2*gridbase_pe,40,50,60,70]
        ...
        self.feature_dims = [1, self.voxel_dim]
        for dim in self.world_size:
            self.feature_dims.append(dim.item())

        self.feature= jt.zeros(self.feature_dims,dtype="float")
    ...
    def sample_ray(self, rays_o, rays_d, near, far, stepsize, is_train=False, **render_kwargs):
        ...
        stepdist = stepsize * self.voxel_size
        ray_pts, mask_outbbox, ray_id, step_id, N_steps, t_min, t_max = render_utils_cuda.sample_pts_on_rays_cuda(
    ...
    def extract_flow(self, test_times, stepsize, testsavedird):
        ...
        dz = dz*dt
        ds = jt.concat([dx,dy,dz], dim=-1).float().detach().cpu().numpy()
    ...
    def div_loss(self):
        ...
        e3 = jt.ones_like(ray_pts_delta)
        e_ds2 = jt.grad(ray_pts_delta, ray_pts,
                                     retain_graph=True)[0]
        #jt.set_grad_enabled(False)
        return e_ds2
    ...
    def execute(self, rays_o, rays_d, viewdirs,times_sel, cam_sel=None,bg_points_sel=None,global_step=None, **render_kwargs):
        ...
        rgb_feature = segment_coo(
                ...
                index=ray_id,
                out=jt.zeros([N, rgb_feature0.shape[-1]]),
                reduce='sum')
        rgb_logit = self.decoder2(rgb_feature.squeeze())
        rgb_marched2 = jt.sigmoid(rgb_logit)
...
class Alphas2Weights(jt.Function):
    ...
    def execute(ctx, alpha, ray_id, N):
        weights, T, alphainv_last, i_start, i_end = render_utils_cuda.alpha2weight_cuda(alpha, ray_id, N)
    ...
    def grad(ctx, grad_weights, grad_last):
        ...
        grad = render_utils_cuda.alpha2weight_backward_cuda(
...
def get_rays(H, W, K, c2w, inverse_y=False, flip_x=False ,flip_y=False, mode='center'):
    W = jt.array(W, dtype="int")
    H = jt.array(H, dtype="int")
    i, j = jt.meshgrid(
        jt.linspace(0, W.item()-1, W.item()),
        jt.linspace(0, H.item()-1, H.item()))  # pytorch's meshgrid has indexing='ij'
    i = i.t().float()
    j = j.t().float()
    if mode == 'lefttop':
        pass
...
def get_rays_of_a_view(H, W, K, c2w, ndc, mode='center'):
    ...
    viewdirs = rays_d / jt.norm(rays_d, dim=-1, keepdim=True)
    if ndc:
...
def get_training_rays_flatten(rgb_tr_ori, times,train_poses, HW, Ks, ndc):
    ...
    N = sum(im.size()[0] * im.size()[1] for im in rgb_tr_ori)
    rgb_tr = jt.zeros([N,3])
...
def get_training_rays_in_maskcache_sampling(rgb_tr_ori, times,train_poses, HW, Ks, ndc, model, render_kwargs):
    ...
    for c2w, img, (H, W), K ,time_one in zip(train_poses, rgb_tr_ori, HW, Ks,times):
        ...
        rays_o, rays_d, viewdirs = get_rays_of_a_view(
                H=H, W=W, K=K, c2w=c2w, ndc=ndc,)
        mask = jt.empty(img.size()[:2], dtype="bool")
        for i in range(0, img.size()[0], CHUNK):
...
def batch_indices_generator(N, BS):
    ...
    idx, top = jt.array(np.random.permutation(N)), 0
    while True:
        if top + BS > N:
            idx, top = jt.array(np.random.permutation(N)), 0
