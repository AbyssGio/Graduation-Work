    ...
    rayspts, maskoutbbox = jt.code(
        ...
        __global__ void sample_pts_on_rays_cuda_kernel(@ARGS_DEF) {
          ...
          if(idx<@in7(0)) {
            ...
            const int offset_p = idx;
            const int offset_r = i_ray;
            const float dist = @in6(0) * i_step;
            const float px = @in0(offset_r,0) + @in1(offset_r,0) * dist;
...
def maskcache_lookup_cuda(
    ...
    out = jt.code(
        ...
        __global__ void maskcache_lookup_cuda_kernel(@ARGS_DEF) {
          ...
          if(i_pt<@in8) {
            const int offset = i_pt;
            const int i = round(@in1(offset,0) * @in3(0) + @in4(0));
