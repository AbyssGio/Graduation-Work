...
def set_id_grid(depth):
	global pixel_coords
	b, h, w = depth.size()
	i_range = jt.array(jt.arange(0, h).view(1, h, 1).expand(1, h, w))  # [1, H, W]
	j_range = jt.array(jt.arange(0, w).view(1, 1, w).expand(1, h, w))  # [1, H, W]
	ones = jt.array(jt.ones(1, h, w))

	pixel_coords = jt.stack((j_range, i_range, ones), dim=1)  # [1, 3, H, W]
...
def cam2pixel(cam_coords, proj_c2p_rot, proj_c2p_tr, padding_mode, rounded=False):
	...
	pixel_coords = jt.stack([X_norm, Y_norm], dim=2)  # [B, H*W, 2]
...
def cam2pixel_cost(cam_coords, proj_c2p_rot, proj_c2p_tr, padding_mode):
	...
	pixel_coords = jt.stack([X_norm, Y_norm], dim=3)  # [B, NNN, H*W, 2]
...
def depth_warp(fdepth, depth, pose, intrinsics, intrinsics_inv, padding_mode='zeros'):
	...
	fdepth_expand = jt.nn.functional.upsample(fdepth_expand, [feat_height, feat_width], mode='bilinear')

	warped_depth = jt.nn.functional.grid_sample(fdepth_expand, src_pixel_coords, mode="nearest",
	                                               padding_mode=padding_mode)
	warped_depth = warped_depth.view(batch_size, feat_height, feat_width)
	# [B, H, W]
	projected_depth = projected_depth.clamp(min=1e-3, max=float(jt.max(warped_depth) + 10))
...
def inverse_warp(feat, depth, pose, intrinsics, intrinsics_inv, padding_mode='zeros'):
	...
	projected_feat = jt.nn.grid_sample(feat, src_pixel_coords, mode='nearest', padding_mode=padding_mode)
...
def inverse_warp_cost(feat, depth, pose, intrinsics, intrinsics_inv, padding_mode='zeros'):
	...
	feat = feat.view(batch_size, 1, channal, feat_height, feat_width).repeat(1, cost_n, 1, 1, 1).view(-1, channal,
	                                                                                                  feat_height,
	                                                                                                  feat_width)
	projected_feat = nn.grid_sample(feat, src_pixel_coords.view(-1, feat_height, feat_width, 2),
	                                                 padding_mode=padding_mode)
