...
class WeightStandardizedConv2d(nn.Conv2d):
    ...
    def execute(self, x):
        ...
        var = reduce(weight, 'o ... -> o 1 1 1', partial(jt.var, unbiased = False))
        normalized_weight = (weight - mean) * (var + eps).rsqrt()

        return nn.conv2d(x, normalized_weight, self.bias, self.stride, self.padding, self.dilation, self.groups)
...
class MeanShift(nn.Conv2d):
    def __init__(self, rgb_range, rgb_mean, rgb_std, sign=-1):
        ...
        self.weight.data = jt.eye(3).view(3, 3, 1, 1)
        self.weight.data.div_(std.view(3, 1, 1, 1))
        self.bias.data = sign * rgb_range * jt.array(rgb_mean)
...
class FocalModulation(nn.Module):
    ...
    def execute(self, x):
        ...
        q, ctx, gates = jt.split(x, (C, C, self.focal_level+1), 1)
...
class FocalModulationBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., drop=0., drop_path=0., 
        ...
        if self.use_layerscale:
            self.gamma_1 = nn.Parameter(layerscale_value * jt.ones((dim)), requires_grad=True)
            self.gamma_2 = nn.Parameter(layerscale_value * jt.ones((dim)), requires_grad=True)
