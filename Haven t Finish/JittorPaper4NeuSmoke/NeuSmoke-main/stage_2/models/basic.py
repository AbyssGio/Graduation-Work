...
class SpatialAttention(nn.Module):
    ...
    def execute(self, x0):
        avg_out = jt.mean(x0, dim=1, keepdims=True)
        max_out, _ = jt.max(x0, dim=1, keepdims=True)
...
class SRNet(nn.Module):
    ...
    def execute(self, tgt_img, tgt_depth, tgt_time):
        ...
        rgbd = jt.concat([tgt_img,tgt_depth],1)
