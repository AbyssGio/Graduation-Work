...
def sample_pdf(bins, weights, n_samples, det=False):
    ...
    below = jittor.max(jittor.zeros_like(inds - 1), inds - 1)
    above = jittor.min((cdf.shape[-1] - 1) * jittor.ones_like(inds), inds)
...
class NeuSRenderer:
    ...
    def render_core_outside(self, rays_o, rays_d, z_vals, sample_dist, nerf, background_rgb=None):
        ...
        dis_to_center = jittor.norm(pts, p=2, dim=-1, keepdim=True).clip(1.0, 1e10)
    ...
    def up_sample(self, rays_o, rays_d, z_vals, sdf, n_importance, inv_s):
        ...
        cos_val, _ = jittor.min(cos_val, dims_mask=-1, keepdims_mask=False)
    ...
    def render_core(self,
        ...
        inv_s = deviation_network(jittor.zeros([1, 3]))[:, :1].clip(1e-6, 1e6)  # Single parameter
        ...
        alpham = alpham.reshape(batch_size, n_samples).clip(0.0, 1.0)
