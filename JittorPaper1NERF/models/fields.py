            ...
            if geometric_init:
                if l == self.num_layers - 2:
                    if not inside_outside:
                        jnn.init.trunc_normal_(lin.weight, mean=np.sqrt(np.pi) / np.sqrt(dims[l]), std=0.0001,
                                               a=np.sqrt(np.pi) / np.sqrt(dims[l]) - 2,
                                               b=np.sqrt(np.pi) / np.sqrt(dims[l]) + 2)
                    ...
                    jnn.init.trunc_normal_(lin.weight[:, :3], 0.0, np.sqrt(2) / np.sqrt(out_dim),
                                           a=0.0 - 3 * np.sqrt(2) / np.sqrt(out_dim),
                                           b=0.0 - 3 * np.sqrt(2) / np.sqrt(out_dim))
                elif multires > 0 and l in self.skip_in:
                    jnn.init.constant_(lin.bias, 0.0)
                    jnn.init.trunc_normal_(lin.weight, 0.0, np.sqrt(2) / np.sqrt(out_dim),
                                           a=0.0 - 3 * np.sqrt(2) / np.sqrt(out_dim),
                                           b=0.0 - 3 * np.sqrt(2) / np.sqrt(out_dim))
