...
class Dataset:
    def __init__(self, conf):
        ...
        self.H, self.W = jittor.attrs(self.images)["shape"][1], jittor.attrs(self.images)["shape"][2]
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        color = oj.Osqueeze(self.images[img_idx])[(pixels_y, pixels_x)]  # batch_size, 3
        # color = self.images[img_idx][(pixels_y, pixels_x)]  # batch_size, 3
        mask = oj.Osqueeze(self.masks[img_idx])[(pixels_y, pixels_x)]  # batch_size, 3
        p = jittor.stack([pixels_x, pixels_y, jittor.ones_like(pixels_y)], dim=-1).float32()  # batch_size, 3
        p = oj.Osqueeze(jnn.matmul(self.intrinsics_all_inv[img_idx, None, :3, :3], p[:, :, None]))  # batch_size, 3
        rays_v = p / jittor.norm(p, p=2, dim=-1, keepdim=True)  # batch_size, 3
        rays_v = oj.Osqueeze(jnn.matmul(self.pose_all[img_idx, None, :3, :3], rays_v[:, :, None]))  # batch_size, 3
        rays_o = oj.Osqueeze(self.pose_all[img_idx, None, :3, 3].expand(rays_v.shape))  # batch_size, 3
        return jittor.concat([rays_o, rays_v, color, mask.unsqueeze(-1)], dim=-1)  # batch_size, 10
    ...
    def gen_rays_between(self, idx_0, idx_1, ratio, resolution_level=1):
        ...
        p = oj.Osqueeze(jnn.matmul(self.intrinsics_all_inv[0, None, None, :3, :3], p[:, :, :, None]))  # W, H, 3
        rays_v = p / jittor.norm(p, p=2, dim=-1, keepdim=True)  # W, H, 3
        trans = self.pose_all[idx_0, :3, 3] * (1.0 - ratio) + self.pose_all[idx_1, :3, 3] * ratio
        pose_0 = self.pose_all[idx_0].detach().numpy()
        pose_1 = self.pose_all[idx_1].detach().numpy()
        ...
        rays_v = oj.Osqueeze(jnn.matmul(rot[None, None, :3, :3], rays_v[:, :, :, None])) # W, H, 3
