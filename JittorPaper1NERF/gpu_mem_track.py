...
if getattr(jittor, "bfloat16", None) is not None:
    dtype_memory_size_dict[jittor.bfloat16] = 16/8
if getattr(jittor, "bool", None) is not None:
    dtype_memory_size_dict[jittor.bool] = 8/8 # pytorch use 1 byte for a bool, see https://github.com/pytorch/pytorch/issues/41571
...
class MemTracker(object):
    ...
    def get_tensors(self):
        for obj in gc.get_objects():
            try:
                if jittor.is_var(obj) or (hasattr(obj, 'data') and jittor.is_var(obj.data)):
                    tensor = obj
                else:
                    continue
                if tensor: #.is_cuda:
...
'''
    def get_allocate_usage(self):
        return jittor.cuda.memory_allocated() / 1024**2
